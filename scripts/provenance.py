...
def _frozen_source(target: str, port: str) -> str:
    """The text of the frozen module, from the same sources `write` records."""
    sources = {}
    for name, path in linked_usermods().items():
        info = describe_repo(path)
        if info is not None:
    ...
    lines = [_FROZEN_HEADER, ""]
    lines.append("BUILT = %r" % datetime.datetime.now().astimezone()
                 .isoformat(timespec="seconds"))
    lines.append("TARGET = %r" % (target or ""))
    lines.append("")
    lines.append("#: {source: git describe}. A name ending in '-dirty' was")
    lines.append("#: built from a tree with uncommitted changes in it, which")
    lines.append("#: is the normal state in this workspace and is recorded")
    lines.append("#: rather than refused -- but it means the commit named is")
    lines.append("#: not the whole of what is in this image.")
    lines.append("SOURCES = {")
    for name in sorted(sources):
        lines.append("    %r: %r," % (name, sources[name]["describe"]))
    lines.append("}")
    lines.append("")
    # The trailing comma is load-bearing: `%` unpacks a tuple on its right,
    # so a one-element OVERLAYS came out as a bare string rather than a
    # tuple, and `for name in OVERLAYS` on a board would have walked its
    # characters.
    lines.append("OVERLAYS = %r"
                 % (tuple(overlays_applied(port)) if port else (),))
    lines.append("")
    lines.append('''

def describe():
    """One line per source, for a REPL or a board-side gate.

        >>> import pydevices_build
        >>> print(pydevices_build.describe())
    """
    rows = ["%s %s" % (TARGET, BUILT)]
    for name in sorted(SOURCES):
        rows.append("  %-20s %s" % (name, SOURCES[name]))
    return "\\n".join(rows)


def contains(source, rev):
    """Whether this image's ``source`` names ``rev``.

    A prefix match on the describe, which is what a gate can ask without a
    git repository underneath it: `describe` carries the abbreviated commit
    after the tag, so a board can refuse an image built from the wrong one.
    Returns None when this image carries no such source, which is a
    ...
    if text is None:
        return None
    rev = str(rev)
    return rev[:7] in text or text in rev
''')
    return "\n".join(lines) + "\n"


def cmd_freeze(args: argparse.Namespace) -> int:
    """Write the module `build_mp.sh` freezes into the firmware.

    Best-effort by contract, and the caller passes `|| true`: a build that
    cannot say what it was made of is worse than one that can, but it is far
    better than a build that does not happen. Every failure here leaves no
    file, and the manifest freezes it only if it is there.
    """
    out = Path(args.out) if args.out else FROZEN_DIR / (FROZEN_MODULE + ".py")
    try:
        text = _frozen_source(args.target, args.port)
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(text, encoding="utf-8")
    except Exception as error:               # noqa: BLE001 - never fatal
        print(f"provenance: no build stamp frozen ({error})", file=sys.stderr)
        return 0
    print(f"Froze {out} ({len(text)} bytes)")
    return 0
...
def main() -> int:
    ...
    freeze = sub.add_parser(
        "freeze", help="write the module build_mp.sh freezes into a firmware")
    freeze.add_argument("--target", default="",
                        help="port/board/variant, recorded as TARGET")
    freeze.add_argument("--port", default="", help="port name, for OVERLAYS")
    freeze.add_argument("--out", default="",
                        help="where to write it (default generated/%s.py)"
                             % FROZEN_MODULE)
